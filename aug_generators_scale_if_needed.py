...
def aug_validation(prob=1.0, img_size=512):
	return Compose([
		LongestMaxSize(max_size=img_size),
		PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT),
	], p=prob)

def aug_resize(prob=1.0, img_size=512):
	return Compose([
		LongestMaxSize(max_size=img_size)
	], p=prob)

def aug_halve(prob=1.0, img_size=512):
	return Compose([
		RandomScale(scale_limit=(0.5, -0.5), p=1.0)
	], p=prob)
	
def aug_pad(prob=1.0, img_size=512):
	return Compose([
		PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT)
	], p=prob)

def aug_daniel_part1(prob=1.0, img_size=512):
	return Compose([
		OneOf([
...
def aug_daniel_part2(prob=1.0, img_size=512):
	return Compose([
		PadIfNeeded(min_height=img_size, min_width=img_size, border_mode=cv2.BORDER_CONSTANT),
...
def imgaug_generator(batch_size = 16, img_size=512):
	...
	augs_halve_probabilistic = aug_halve(prob=.25, img_size=img_size)
...
#			mask = resize(mask, (img_size, img_size), preserve_range=True) #np.float32 [0.0, 255.0]
			...
			if img.shape[0] > img_size or img.shape[1] > img_size:
				...
				#Crop if still larger than 384 - don't want to sacrificae accuracy for input flexibility
				if img.shape[0] > img_size or img.shape[1] > img_size:
					row_min = 0
					row_max = min(img_size, img.shape[0])
					column_min = 0
					column_max = min(img_size, img.shape[1])
			...
			for j in range(augs_per_image):
				...
				mask_edge = cv2.Canny(mask_aug_1, 1000, 1000) #Only accept sharp edges (gradient can be expected to be >255, 1000 as emperical safety margin
