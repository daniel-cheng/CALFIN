...
def shift_scale_rotate(img, angle, scale, dx, dy):
    ...
    #get maximum rectangle that doesn't include invalid pixels
    x = sorted(box1[:,0].astype(int))
    y = sorted(box1[:,1].astype(int))
    minX = max(x[1] + 1, 0)
    maxX = min(x[2] - 1, width-1)
    minY = max(y[1] + 1, 0)
    maxY = min(y[2] - 1, height-1)
...
def elastic_transform_fast(image, alpha, sigma, alpha_affine, random_state=None):
    ...
    y = sorted(box1[:,1].astype(int))
    minX = max(x[1] + 1, 0)
    maxX = min(x[2] - 1, shape[0]-1)
    minY = max(y[1] + 1, 0)
    maxY = min(y[2] - 1, shape[1]-1)
    #TODO: When does maxX - minX = 0?
