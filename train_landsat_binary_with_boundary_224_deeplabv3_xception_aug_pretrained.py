...
def imgaug_generator(batch_size = 4, img_size=224):
    train_data_path = 'landsat_raw_boundaries/train_partial'
...
#   preprocess_input = get_preprocessing(BACKBONE)
    ...
    clr_triangular = CyclicLR(mode='triangular', step_size=4000, base_lr=2e-5, max_lr=8e-5)
    callbacks_list = [
...
#           layer.trainable = False
    ...
    model.compile(optimizer=Adam(lr=5e-5), loss=bce_jaccard_loss, metrics=['binary_crossentropy', iou_score, 'accuracy'])
    model.summary()
#       model.load_weights('landsat_weights_with_boundary_224_deeplabv3_xception_e17_iou0.9695.h5')
    model.load_weights('landsat_weights_with_boundary_224_deeplabv3_xception_e40_iou0.7419.h5')


    print('-'*30)
    print('Fitting model...')
    print('-'*30)
    train_generator = imgaug_generator(16, img_size)
