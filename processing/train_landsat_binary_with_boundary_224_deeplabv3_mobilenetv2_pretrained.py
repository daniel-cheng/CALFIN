...
from data_landsat_binary_with_boundary_224 import load_train_data, create_unagumented_data_from_image, load_test_data_from_image, create_train_data_from_directory, load_validation_data
...
def predict(model, image, mask):
	...
	image_resize, mask_resize = create_unagumented_data_from_image(image, mask)
	#Cast img to float32, perform imagenet preprocesssing (~0.45 means, -[1,1])
	#Resize to 1x224x224x1
	imgs_test, nr, nc, nR, nC = load_test_data_from_image(image_resize, stride)
...
#	full_image = full_image[0:nr, 0:nc].astype(np.uint8)
	return full_image, image_resize, mask_resize
...
def imgaug_generator(batch_size = 4):
	...
	while True:
		...
		for i in range(images_per_metabatch):
			...
			img = resize(img, (224, 224), preserve_range=True)  #np.float32 [0.0, 65535.0]
			mask = resize(mask, (224, 224), preserve_range=True) #np.float32 [0.0, 255.0]
...
#	preprocess_input = get_preprocessing(BACKBONE)
	...
	model_checkpoint = ModelCheckpoint('landsat_weights_with_boundary_224_deeplabv3_mobilenetv2_e{epoch:02d}_iou{val_iou_score:.4f}.h5', monitor='val_iou_score', save_best_only=False)
...
#		model = load_model('landsat_weights_with_boundary_224_' + hyperparameters_string + '.h5')
	...
	#	print('-'*30)
		model.load_weights('landsat_weights_with_boundary_224_0.0085.h5')
...
#		base_model = Unet(BACKBONE, encoder_weights='imagenet')
		base_model = Deeplabv3(input_shape=(224,224,3), classes=1)
		#Don't freeze??
...
#			layer.trainable = False
		...
		model.summary()
		model.load_weights('landsat_weights_with_boundary_224_0.7123.h5')
...
#	model.save('landsat_weights_with_boundary_224_deeplabv3_mobilenetv2_val-0.9837h5')
#
	print('-'*30)
	print('Predicting masks on test data...')
	print('-'*30)
	pred_path = 'landsat_preds_boundaries/validation_full'
		...
		if '_mask' not in name and '_pred' not in name and '_bqa' not in name and '_mtl' not in name:
			...
			if os.path.isfile(path):
				...
				for angle in [0, 90, 180, 270]:
					...
					result, image_original, mask_original = predict(model, rot_img, mask).astype('float32')
					result = rotate(result, -angle, mode='reflect', preserve_range=True).astype(np.uint8)
					image_original = rotate(image_original, -angle, mode='reflect', preserve_range=True).astype(np.uint8)
				...
				imsave(os.path.join(pred_path, name[0:-4] + '_raw.png'), image_original)
				imsave(os.path.join(pred_path, name[0:-4] + '_mask.png'), mask_original)
