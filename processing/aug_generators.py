...
def aug_daniel(prob=0.8,img_size=224):
    return Compose([
        RandomRotate90(p=0.5),
        Transpose(p=0.5),
        Flip(p=0.5),
...
def imgaug_generator(batch_size = 16, img_size=224):
    ...
    augs = aug_daniel(img_size=img_size)
    ...
    while True:
        ...
        for i in range(images_per_metabatch):
            ...
            img_resized = (img_resized * (255.0 / img_resized.max())).astype(np.uint8)

            #Resize/pad image to max 224
            #dat_resize = augs_resize(image=img, mask=mask)
            #img_resized = dat_resize['image']
            #mask_resized = dat_resize['mask']

            #Run each image through 8 random augmentations per image, but favor rarer examples.
            if image_name.split('_')[0] not in ['Jakobshavn', 'Upernavik', 'Hayes', 'Rink-Isbrae', 'Kong-Oscar', 'Kangiata-Nunata']:
                augs_per_image = 24
            else:
                augs_per_image = 8
            for j in range(augs_per_image):
                #Augment image.
                dat = augs(image=img_resized, mask=mask_resized)
                img_aug = dat['image']
                mask_aug = dat['mask']
                
                #Calculate edge from mask and dilate.
                mask_aug = mask_aug.astype(np.uint8)
                mask_edge = cv2.Canny(mask_aug, 1000, 1000) #Only accept sharp edges (gradient can be expected to be >255, 1000 as emperical safety margin
                ...
                if (img_aug.shape != (img_size, img_size) or mask_aug.shape != (img_size, img_size)):
                    img_aug = resize(img_aug, (img_size, img_size), preserve_range=True).astype(np.float32)  #np.float32 [0.0, 65535.0]
                    mask_aug = resize(mask_aug, (img_size, img_size), preserve_range=True).astype(np.float32) #np.float32 [0.0, 255.0]
