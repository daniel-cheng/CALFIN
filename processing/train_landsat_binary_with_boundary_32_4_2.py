...
def conv_block(m, dim, acti, bn, res, do=0):
	n = Conv2D(dim, 3, activation=acti, dilation_rate=1, padding='same', kernel_regularizer=l2(0.00001))(m)
	n = BatchNormalization()(n) if bn else n
	n = Dropout(do)(n) if do else n
	n = Conv2D(dim, 3, activation=acti, dilation_rate=1, padding='same', kernel_regularizer=l2(0.00001))(n)
	n = BatchNormalization()(n) if bn else n
...
def level_block(m, dim, depth, inc, acti, do, bn, mp, up, res):
	if depth > 0:
		...
		if up:
			m = UpSampling2D()(m)
			m = Conv2D(dim, 2, activation=acti, padding='same', kernel_regularizer=l2(0.00001))(m)
		else:
