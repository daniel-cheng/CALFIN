...
class Resize(DualTransform):
    def __init__(self, prob=1.0, width=256, height=256):
        super().__init__(prob)
        self.width = width
        self.height = height

    def apply(self, img):
        return F.resize(img, self.width, self.height)
...
def aug_victor(prob=0.9):
    return Compose([
        ...
        ShiftScaleRotate(shift_limit=.0, scale_limit=0.2, rotate_limit=20, prob=.6),
        OneOf([
            Distort1(prob=.25),
            Distort2(prob=.25),
