...
"""
import os,sys,math,urllib.request,urllib.error,urllib.parse,urllib.request,urllib.parse,urllib.error,time,math,shutil
import subprocess
import optparse
import datetime
import csv
...
def connect_earthexplorer_no_proxy(usgs):
	...
	#print("https://ers.cr.usgs.gov/login?" + params)
	request = urllib.request.Request("https://ers.cr.usgs.gov/login", data=params.encode(), headers={})
	f = urllib.request.urlopen(request)
...
def downloadChunks(url,rep,nom_fic):
	...
	try:
		...
		if ('text/html' in req.getheader('Content-Type')):
		...
		#check if need to redownload
		file_path = rep + '/' + nom_fic
		print(file_path)
		if os.path.exists(file_path):
			size_on_disk = os.stat(file_path).st_size
			if total_size == size_on_disk:
				return rep,nom_fic
			else:
				print('Size mismatch. total_size:', total_size, 'size_on_disk:', size_on_disk)
...
##########Telechargement par liste
	...
	if options.option=='liste':
		for scene_list in glob.glob(options.fic_liste):
			with open(scene_list, 'r') as f:
				...
				for ligne in lignes:
					try:
						...
						elif produit.startswith('LT5'):
							repert='12266'
							stations=['ASN','GLC','ASA','KIR','MOR','KHC', 'PAC', 'KIS', 'CHM', 'LGS', 'MGR', 'COA', 'MPS','JSA']	
						elif produit.startswith('LM5') or produit.startswith('LM4') or produit.startswith('LM3') or produit.startswith('LM2') or produit.startswith('LM1'):
							repert='14661'
							stations=['ASN','GLC','ASA','KIR','MOR','KHC', 'PAC', 'KIS', 'CHM', 'LGS', 'MGR', 'COA', 'MPS','JSA']	
						if not os.path.exists(rep):
							os.mkdir(rep)
						...
						downloadChunks(url,rep,site+'.tar.gz')
