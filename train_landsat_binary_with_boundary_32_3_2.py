...
from data_landsat_binary_with_boundary import load_train_data, load_test_data_from_image, create_train_data_from_directory, load_validation_data
...
def conv_block(m, dim, acti, bn, res, do=0):
	n = Conv2D(dim, 3, activation=acti, dilation_rate=2, padding='same', kernel_regularizer=l2(0.0001))(m)
	n = BatchNormalization()(n) if bn else n
	n = Dropout(do)(n) if do else n
	n = Conv2D(dim, 3, activation=acti, dilation_rate=2, padding='same', kernel_regularizer=l2(0.0001))(n)
	n = BatchNormalization()(n) if bn else n
...
if __name__ == '__main__':
	...
	model_checkpoint = ModelCheckpoint('landsat_weights_with_boundary_256_' + hyperparameters_string + '_{val_dice_coef:.5f}.h5', monitor='val_binary_crossentropy', save_best_only=False)
	#try:
		#print('-'*30)
		#print('Loading model...')
		#print('-'*30)
	...
	print('-'*30)
	model = UNet((img_rows,img_cols,1), start_ch=hyperparameters[0], depth=hyperparameters[1], inc_rate=hyperparameters[2], activation='relu')
	model.compile(optimizer=Adam(lr=1e-5), loss=dice_coef_loss, metrics=['binary_crossentropy', dice_coef])

	print('-'*30)
	print('Loading saved weights...')
	print('-'*30)
	#model.load_weights('landsat_weights_with_boundary_256_32-3-2_0.39524.h5')
...
#				imsave(os.path.join(pred_path, name[0:-4] + '_raw.png'), image)
	...
	for i in range(20):
		...
		np.save('landsat_imgs_train_boundaries.npy', imgs_train)
		np.save('landsat_imgs_mask_train_boundaries.npy', imgs_mask_train)
