...
def initialize(img_size, source_root_path=r"../processing/testing/*B[0-9].tif", dest_root_path=r"../outputs/testing"):
    ...
    validation_files = glob.glob(source_root_path)
    ...
    settings['mask_detection_ratio_threshold'] = 32 #if land/ice area is 32 times bigger than ocean/melange, classify as no front/unconfident prediction
...
# add back in more upsampling 2d, find where it breaks
    if len(sys.argv) > 1:
        ...
        dest_dir = sys.argv[2]
        settings, metrics = initialize(img_size, source_dir, dest_dir)
    else:
