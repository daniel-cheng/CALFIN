...
def conv_block(m, dim, acti, bn, res, do=0):
	n = Conv2D(dim, 3, activation=acti, dilation_rate=1, padding='same', kernel_regularizer=l1_l2(0.00001, 0.00001))(m)
	n = BatchNormalization()(n) if bn else n
	n = Dropout(do)(n) if do else n
	n = Conv2D(dim, 3, activation=acti, dilation_rate=1, padding='same', kernel_regularizer=l1_l2(0.00001, 0.00001))(n)
	n = BatchNormalization()(n) if bn else n
...
def level_block(m, dim, depth, inc, acti, do, bn, mp, up, res):
	if depth > 0:
		...
		if up:
			m = UpSampling2D()(m)
			m = Conv2D(dim, 2, activation=acti, padding='same', kernel_regularizer=l1_l2(0.00001, 0.00001))(m)
		else:
...
if __name__ == '__main__':
	...
	#	print('-'*30)
		...
		model.compile(optimizer=Adam(lr=1e-3), loss='binary_crossentropy', metrics=['binary_crossentropy', dice_coef])
		model.load_weights('landsat_weights_with_boundary_256_32-4-2_0.16987.h5')
	...
	#				rot_img = rotate(image, angle, mode='reflect', preserve_range=True)
