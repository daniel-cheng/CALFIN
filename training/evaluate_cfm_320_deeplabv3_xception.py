...
from data_cfm_320 import load_validation_data
...
#	model.summary()
	model.load_weights('cfm_weights_320_e02_iou0.0065.h5')
	
	print('-'*30)
	print('Fitting model...')
	print('-'*30)

#	images = model.predict(x=validation_data[0],
#				batch_size=1,
#				verbose=1)
...
#		plt.imshow(images[i,:,:,0])
	...
	images = model.evaluate_generator(train_generator,
				steps=40,
				verbose=1)
	for i in range(images.shape[0]):
		plt.figure(i)
