...
img_size = 320
stride = int((img_size + img_size) / 2 / 2) #1/2 of img_window square
smooth = 1.
data_path = 'data/'
pred_path = 'preds/'
temp_path = 'temp/'
...
def predict(model, image):
	...
	patches, _ = create_unagumented_data_from_image(image, None) #np.float32 [-1.0, 1.0] (mean ~0.45), shape = (1, 320, 320, 1) #np.float32 [0.0, 1.0] (mean<0.5), shape = (1, 320, 320, 1)
	#Cast img to float32, perform imagenet preprocesssing (~0.45 means, -[1,1])
	#Resize to 1x320x320x1
...
if __name__ == '__main__':
	...
	model.summary()
#		model.load_weights('landsat_weights_with_boundary_320_deeplabv3_xception_e17_iou0.9695.h5')
#		model.load_weights('landsat_weights_with_boundary_320_deeplabv3_xception_e20_iou0.8794.h5')
			
	print('-'*30)
	print('Predicting masks on test data...')
	print('-'*30)
	pred_path = 'landsat_preds_boundaries/test_full_320'
