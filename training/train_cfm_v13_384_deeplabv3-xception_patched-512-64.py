...
if __name__ == '__main__':
    ...
    model_checkpoint = ModelCheckpoint('cfm_weights_patched_dual_wide_x65_' + str(img_size) + '_e{epoch:02d}_iou{val_weighted_iou_score:.4f}.h5', monitor='val_weighted_iou_score', save_best_only=False, save_weights_only=True)
    ...
    if tf.__version__ < '2.0.0':
        ...
        def bce_ln_jaccard_loss(gt, pr, bce_weight=1.0, smooth=SMOOTH, per_image=True):
            bce = tf.math.reduce_mean(binary_crossentropy(gt[:,:,:,0], pr[:,:,:,0]))*25/26 + tf.math.reduce_mean(binary_crossentropy(gt[:,:,:,1], pr[:,:,:,1]))/26
            loss = bce_weight * bce - tf.math.log(iou_score(gt[:,:,:,0], pr[:,:,:,0], smooth=smooth, per_image=per_image))*25/26 - tf.math.log(iou_score(gt[:,:,:,1], pr[:,:,:,1], smooth=smooth, per_image=per_image))/26
            return loss
        ...
        def deviation(gt, pr, smooth=SMOOTH2, per_image=True):
            mismatch = tf.math.reduce_sum(tf.math.abs(gt[:,:,:,1] - pr[:,:,:,1]), axis=[1, 2]) #(B)
            length = tf.math.reduce_sum(gt[:,:,:,0], axis=[1, 2]) #(B)
            deviation = mismatch / (length + smooth) #(B)
            mean_deviation = tf.math.reduce_mean(deviation) / 3.0 #- (account for line thickness of 3 at 224)
            return mean_deviation
    ...
    model = Deeplabv3(input_shape=(img_size, img_size,3), classes=16, OS=16, backbone='xception', weights=None, atrous_rates = (2, 4, 6, 9, 12))

    model.compile(optimizer=AdamAccumulate(lr=1e-4, accum_iters=8), loss=bce_ln_jaccard_loss, metrics=[weighted_iou_score, edge_iou_score, mask_iou_score, deviation])
    model.summary()
    ...
    train_generator = imgaug_generator_patched(2, img_size=full_size, patch_size=img_size, patch_stride=stride, steps_per_epoch=steps_per_epoch)
