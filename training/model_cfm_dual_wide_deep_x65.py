...
#			atrous_rates = (6, 12, 18)
	...
	b4 = Conv2D(256, (1, 1), padding='same',
				use_bias=False, kernel_regularizer=l1_l2(1e-6, 1e-6), name='image_pooling_max')(b4)
	b4 = BatchNormalization(name='image_pooling_max_BN', epsilon=1e-5)(b4)
	b4 = Activation('elu')(b4)
	b4 = BilinearUpsampling((2,2))(b4)

	b7 = AveragePooling2D(pool_size=(int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))))(x)
	b7 = Conv2D(256, (1, 1), padding='same',
				use_bias=False, name='image_pooling_avg')(b7)
	b7 = BatchNormalization(name='image_pooling_avg_BN', epsilon=1e-5)(b7)
	b7 = Activation('elu')(b7)
	b7 = BilinearUpsampling((int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))))(b7)
	...
	if backbone == 'xception':
		...
		x = Concatenate()([b4, b0, b1, b2, b3, b5, b6, b7])
	else:
		x = Concatenate()([b4, b0])
