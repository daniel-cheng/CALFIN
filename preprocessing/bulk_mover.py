...
# total = 0
# total_bad = 0
# for domain in os.listdir(root_dest_path):
#     dest_path = os.path.join(root_dest_path, domain)
#     ref_list = os.listdir(dest_path)
#     for file_path in glob.glob(os.path.join(root_dest_path, domain, '*_overlay_comparison.png')):
#         file_name = os.path.basename(file_path)
#         target_file_name = file_name.split('_comparison.png')[0] + '_front.png'
#         source_file_path = os.path.join(root_source_path, domain, target_file_name)
#         dest_file_path = os.path.join(root_dest_path, domain, target_file_name)
#         # shutil.copy2(source_file_path, dest_file_path)
#         os.remove(file_path)

root_source_path = r'D:\Daniel\Documents\Github\CALFIN Repo\outputs\production_staging'
total = 0
total_bad = 0
for file_path in glob.glob(os.path.join(root_source_path, 'all', '*_closed*')):
    print(file_path)
    # break
    os.remove(file_path)
for file_path in glob.glob(os.path.join(root_source_path, 'domain', '*', '*_closed*')):
    print(file_path)
    # break
    os.remove(file_path)
for file_path in glob.glob(os.path.join(root_source_path, 'quality_assurance', '*', '*polygon*')):
    print(file_path)
    # break
    os.remove(file_path)
