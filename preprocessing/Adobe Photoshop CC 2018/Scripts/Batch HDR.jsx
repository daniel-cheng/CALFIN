    ...
    if(sourceFolder != null && outputFolder != null && sourceFolder.exists && outputFolder.exists && numberOfBrackets > 0)
    {
        ...
        for(var index = 0;  index < numberOfFiles; index++)
        {
            if((index % numberOfBrackets) == numberOfBrackets - 1)
            {
                ...
                if(userCanceled) break;
                doSaveFile(outputFolder.absoluteURI + "/" + files[index].name.substring(0, files[index].name.length - 4) + '_hdr.png' );
...
function doSaveFile(filename)
{
	...
	else if(saveType == "PNG")
    {
        ...
        activeDocument.saveAs(new File(filename), pngSaveOptions, true /*Save As Copy*/, Extension.LOWERCASE /*Append Extention*/);
    }
...
function promptUser()
{
    ...
    outputBitDepthDropDown.selection = 1;
