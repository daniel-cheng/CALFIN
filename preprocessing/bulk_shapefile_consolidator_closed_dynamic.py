...
#			if not('Upernavik' in domain or 'Rink-Isbrae' in domain or 'Inngia' in domain or 'Umiammakku' in domain):
			...
			with rasterio.open(fjord_boundary_file_path) as fjord_boundary_tif:
				...
				confidence_mask = np.zeros(fjord_boundary_mask.shape)
				candadite_list = np.zeros(fjord_boundary_mask.shape)
...
# 							counter += 2
						...
						candadite_list += mask
				#Need some way to find outliers
				#Maybe mask out all values above certaint percentage, then count how many "(pseudo)uniques" each front has
				#if it exceeds some threshold, the front is too "unique" and likely has errors
				#invert, then sum?
				#penalize uniques and penalize lack of commons (two independdent measures, both must trigger)
# 				plt.imshow(confidence_mask)
				for mask in candadite_list:
					intersection = (mask) * confidence_mask
					nonzero_fraction = np.shape(np.nonzero(intersection))[1] / intersection.size
					nonzero_threshold = 0.03
					if nonzero_fraction > nonzero_threshold:
						print('skipping, nonzero_fraction > nonzero_threshold:', nonzero_fraction)
