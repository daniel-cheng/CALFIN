...
from data_landsat_binary_with_boundary_256 import create_unagumented_data_from_image, load_validation_data
...
if __name__ == '__main__':
	...
	clr_triangular = CyclicLR(mode='triangular2', step_size=4000, base_lr=1e-2, max_lr=1e-4)
	callbacks_list = [
	...
	base_model = Deeplabv3(input_shape=(img_rows,img_cols,3), classes=1, backbone='mobilenetv2')
	...
	#model.compile(optimizer=SGD(lr= 0.01, momentum =0.9, decay = 0.0001, nesterov = True), loss=bce_jaccard_loss, metrics=['binary_crossentropy', iou_score, 'accuracy'])
	model.summary()
